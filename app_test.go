...
import (
	...
	"github.com/plum330/kratos/v2/transport/grpc"
	"github.com/plum330/kratos/v2/transport/http"
)
...
func TestApp_Endpoint(t *testing.T) {
	v := []string{"https://plum330.dev", "localhost"}
...
func TestApp_buildInstance(t *testing.T) {
	want := struct {
		...
		endpoints: []string{"https://plum330.dev", "localhost"},
	}
	var endpoints []*url.URL
	...
	tests := []fields{
		{
			id:       "1",
			name:     "kratos-v1",
			instance: &registry.ServiceInstance{Endpoints: []string{"https://plum330.dev", "localhost"}},
			metadata: map[string]string{},
			version:  "v1",
			want: struct {
				id       string
				...
				id: "1", version: "v1", name: "kratos-v1", endpoint: []string{"https://plum330.dev", "localhost"},
				metadata: map[string]string{},
			},
		},
		{
			...
			metadata: map[string]string{"kratos": "https://github.com/plum330/kratos"},
			version:  "v2",
			want: struct {
				id       string
				version  string
