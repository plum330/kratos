...
func TestDefaultDecoder(t *testing.T) {
	...
	if !reflect.DeepEqual(target, map[string]any{"service": []byte("config")}) {
		t.Fatal(`target is not equal to map[string]any{"service": "config"}`)
	}
	...
	if !reflect.DeepEqual(map[string]any{
		...
		t.Fatal(`target is not equal to map[string]any{"service": map[string]any{"name": map[string]any{"alias": []byte("2233")}}}`)
	}
}
